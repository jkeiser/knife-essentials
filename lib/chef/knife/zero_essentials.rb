      ...
      class ChefFSDataStore
        ...
        def get(path)
          if MEMORY_PATHS.include?(path[0])
            ...
            begin
              ChefFS::FileSystem.resolve_path(chef_fs, "#{path.join('/')}.json").read
        ...
        def delete(path)
          if MEMORY_PATHS.include?(path[0])
            ...
            begin
              ChefFS::FileSystem.resolve_path(chef_fs, "#{path.join('/')}.json").delete
            rescue ChefFS::FileSystem::NotFoundError => e
              raise ChefZero::DataStore::DataNotFoundError.new(path, e)
            end
          end
        ...
        def delete_dir(path, *options)
          if MEMORY_PATHS.include?(path[0])
            ...
            begin
              ChefFS::FileSystem.resolve_path(chef_fs, path.join('/')).delete(options.include?(:recursive))
            rescue ChefFS::FileSystem::NotFoundError => e
              raise ChefZero::DataStore::DataNotFoundError.new(path, e)
            end
          end
        ...
        def list(path)
          if MEMORY_PATHS.include?(path[0])
            ...
            begin
              ChefFS::FileSystem.resolve_path(chef_fs, path.join('/')).children.map { |c| remove_dot_json(c) }.sort
            rescue ChefFS::FileSystem::NotFoundError => e
              raise ChefZero::DataStore::DataNotFoundError.new(path, e)
            end
          end
        ...
        def exists?(path)
          if MEMORY_PATHS.include?(path[0])
            ...
            ChefFS::FileSystem.resolve_path(chef_fs, "#{path.join('/')}.json").exists?
          end
        end
        ...
        def exists_dir?(path)
          if MEMORY_PATHS.include?(path[0])
            ...
            ChefFS::FileSystem.resolve_path(chef_fs, path.join('/')).exists?
          end
        end
