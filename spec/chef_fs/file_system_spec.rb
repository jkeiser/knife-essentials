    ...
    context 'get_error_cause' do
      context 'error has a cause attribute and cause attribute is instance of Net::HTTPServerException' do
        it 'return error cause' do
          ...
          error = ChefFS::FileSystem::OperationFailedError.new(:write, self, exception)
          ChefFS::FileSystem.get_error_cause(error).should == response_body
        end
      end

      context 'error has not a cause property' do
        it 'return nil' do
          error = ChefFS::FileSystem::OperationFailedError.new(:write, self)
          ChefFS::FileSystem.get_error_cause(error).should == nil
        end
      end
    end
  end
